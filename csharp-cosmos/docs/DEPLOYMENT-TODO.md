...
| Item | Description | Where |
|------|-------------|--------|
| **Cosmos DB** | API needs Cosmos endpoint and database name to use CosmosClient and DatabaseInitializer. | In Azure: usually provided via App Service app settings from Bicep outputs (`AZURE_COSMOS_ENDPOINT`, `AZURE_COSMOS_DATABASE_NAME`). For local dev: set in `appsettings.Development.json` or env, or leave empty to skip Cosmos. |
| **Azure SQL connection string** | API uses `ReferenceDataDbContext` when a connection string is present. | Set `AzureSql:ConnectionString` or `AZURE_SQL_CONNECTION_STRING` (server, database, user, password; consider storing password in Key Vault and referencing from App Service). Not set by Bicep. |
